...
let currentUser = JSON.parse(localStorage.getItem('quizlyUser') || 'null');
...
const renderQuestion = () => {
    ...
    document.querySelector('.current-question-number').textContent = questionIndexHistory.length;
    document.querySelector('.progress-bar').style.width = `${(questionIndexHistory.length / numberOfQuestions) * 100}%`;
...
const startQuiz = () => {
    ...
    if (levelActive) quizCategory = levelActive.textContent.trim().toLowerCase().replace('medium', 'moderate');
...
const renderLeaderboard = () => {
    document.querySelector('.score-value').textContent = correctAnswerCount;
};

const originalShowQuizResult = showQuizResult;
showQuizResult = () => { originalShowQuizResult(); renderLeaderboard(); };
...
loginForm.addEventListener('submit', (event) => {
    ...
    currentUser = { name: formData.get('playerName').trim(), email: formData.get('playerEmail').trim() };
    localStorage.setItem('quizlyUser', JSON.stringify(currentUser));
    showSetup();
});

document.querySelectorAll('.category-option').forEach((option) => {
    option.addEventListener('click', () => {
        document.querySelectorAll('.category-option').forEach((category) => category.classList.remove('active'));
        option.classList.add('active');
        quizCategory = option.dataset.category;
    });
});